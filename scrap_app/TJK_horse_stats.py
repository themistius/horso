...
# Constants
HEADERS_URL = 'https://www.tjk.org/TR/YarisSever/Query/Page/AtIstatistikleri#'
DATA_URL = 'https://www.tjk.org/TR/YarisSever/Query/DataRows/AtIstatistikleri'
...
def save_to_database(cursor, data_list, column_names):
    ...
    insert_sql = f'''
    INSERT INTO horses ({', '.join(['"' + name + '"' for name in column_names])})
...
def scrape_page(page_number, column_names, data_list):
    url = f"{DATA_URL}?PageNumber={page_number}&Sort=derece1+DESC&X-Requested-With=XMLHttpRequest"
